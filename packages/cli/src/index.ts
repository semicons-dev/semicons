...
import type { NormalizedRegistry, NormalizedToken } from '@semicons/core';
import { loadConfig, loadRemoteConfig } from './lib/config-loader.js';
import {
  toRegistryJSON,
  toRegistrySchema,
  filterLocalTokens,
...
export function run() {
  ...
  program
    ...
    .option('-r, --remote <url>', 'load config from remote URL')
...
async function cmdGenerate(options: GenerateOptions) {
  ...
  if (options.remote) {
    const result = await loadRemoteConfig(options.remote, options.strict ? { onWarning: (w: any) => {} } : undefined);
    ...
    const { registry } = normalizeConfig(result.config, normalizeOptions);
    await doGenerate(registry, options, configDir, warnings, isRemote);
    return;
  }
  ...
  if (!configPath) {
    throw new Error('No config file found. Use --config, --remote, or create semicons.config.mjs');
  }
  ...
  const { registry } = await loadConfig(configPath, normalizeOptions);
  configDir = path.dirname(path.resolve(configPath));
  await doGenerate(registry, options, configDir, warnings, isRemote);
}

async function doGenerate(registry: NormalizedRegistry, options: GenerateOptions, configDir: string, warnings: any[], isRemote: boolean) {
  const outDir = path.resolve(options.out || 'src/icons.generated');
  await fs.mkdir(outDir, { recursive: true });

  if (isRemote) {
    console.log(`[generate] Using remote configuration`);
  }
  ...
  const tokenNames = registry.tokens.map((t: NormalizedToken) => t.name);
  const typesContent = `// Auto-generated by semicons
export type IconName = ${tokenNames.map((n: string) => `'${n}'`).join(' | ')};
...
${registry.tokens.map((t: NormalizedToken) => `  '${t.name}': {
    name: '${t.name}',
...
`;
  ...
  const typesDtsContent = `// Auto-generated by semicons
export type IconName = ${tokenNames.map((n: string) => `'${n}'`).join(' | ')};
...
`;
  await fs.writeFile(path.join(outDir, 'types.d.ts'), typesDtsContent);
...
`;
  ...
  const inlineDtsContent = `// Auto-generated by semicons
export interface InlineIconData {
  svg: string;
...
`;
  await fs.writeFile(path.join(outDir, 'inline.d.ts'), inlineDtsContent);
